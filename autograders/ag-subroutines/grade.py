#!/usr/bin/env python3
"""
ag-subroutines: command-line autograder for the ATCS Compilers
Subroutines lab.

Usage:
    ag-subroutines STUDENT.zip [-o OUTPUT_DIR]
    ag-subroutines STUDENTS_DIR/ [-o OUTPUT_DIR]
...
The autograder needs:
    ...
    - a JRE on the PATH (just `java`; MARS handles assembly itself)
...
def _build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="ag-subroutines",
        description="ATCS Compilers Subroutines-lab autograder. "
    ...
    p.add_argument("--java", default=shutil.which("java") or "java",
                   help="Path to the java executable (auto-detected by "
                        "default). Used to launch MARS in CLI mode.")
...
def _report_path(out_dir: Path, student_slug: str,
    ...
    """Compose a unique '<student-slug>-subroutines-report.pdf' path.

    Same disambiguation rule as ag-mips: append "-2", "-3", ... if two
    submissions collide (e.g. two students sharing a first/last name).
    """
...
def _grade_one(zip_path: Path, out_dir: Path, args,
    ...
    print(f"[ag-subroutines] Grading: {zip_path.name}")
    ...
    try:
        ...
        print(f"[ag-subroutines]   EXTRACTION FAILED: {exc}", file=sys.stderr)
        ...
        try:
            ...
            print(f"[ag-subroutines]   -> {out_path}  (error stub)")
    ...
    print(f"[ag-subroutines]   student: {submission.student_name!r} "
          f"-> {out_path.name}")
    ...
    try:
        ...
        tb = traceback.format_exc()
        print(f"[ag-subroutines]   GRADING FAILED (emitting stub): {exc}",
              file=sys.stderr)
        try:
            render_error_stub(zip_path, out_path, tb)
            print(f"[ag-subroutines]   -> {out_path}  (error stub)")
    ...
    try:
        ...
        print(f"[ag-subroutines]   -> {out_path}  ({graded.percent:.1f}%)")
        if roster is not None:
            roster.append((graded.submission.student_name,
                           graded.percent, out_path.name))
        return out_path
    except Exception as exc:
        print(f"[ag-subroutines]   RENDER FAILED: {exc}", file=sys.stderr)
        traceback.print_exc()
        return None
    finally:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(zips) > 1 and roster:
        ...
        try:
            render_overall(roster, overall_path, lab_name="Subroutines Lab")
            print(f"[ag-subroutines] Batch summary: {overall_path}")
        except Exception as exc:
            print(f"[ag-subroutines] Batch summary FAILED: {exc}",
                  file=sys.stderr)

    if failed:
        print(f"[ag-subroutines] Done. {failed}/{len(zips)} report(s) failed.")
        return 1
    print(f"[ag-subroutines] Done. {len(zips)} report(s) written to {out_dir}.")
    return 0
