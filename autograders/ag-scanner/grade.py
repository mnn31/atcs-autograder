#!/usr/bin/env python3
"""
ag-scanner: command-line autograder for the ATCS Compilers SCANNER lab.

Usage:
    ag-scanner STUDENT.zip [-o OUTPUT_DIR]
    ag-scanner STUDENTS_DIR/ [-o OUTPUT_DIR]

Either pass a single .zip file or a directory containing many .zip files;
one PDF report is produced per submission. Reports land in OUTPUT_DIR
(default: ./reports/) and are named after the student's @author tag,
falling back to the zip basename. A batch summary overall.pdf is
generated when more than one submission is processed.

Requires:
    - python 3.8+ with reportlab and javalang installed
...
from agcore.grader import grade                            # noqa: E402
...
def _build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="ag-scanner",
        description="ATCS Compilers Scanner-lab autograder. Produces a "
                    "colour-coded PDF report for each student submission zip.",
    )
    ...
    p.add_argument("--javac", default=shutil.which("javac") or "javac",
                   help="Path to the javac executable.")
...
def _report_path(out_dir: Path, student_slug: str,
    ...
    """Compose a unique <slug>-scanner-report.pdf path; bumps -2/-3/...
    when two submissions collide on @author or fall back to "student".
    """
...
def _grade_one(zip_path: Path, out_dir: Path, args,
    ...
    """Grade a single zip and return the path of the resulting PDF.

    Always produces a PDF (even on extraction or rendering failure) so
    the teacher has something to look at per submission. Silent drops
    are worse than ugly reports.
    """
    cfg = config.build_config(java_exe=args.java, javac_exe=args.javac)
    print(f"[ag-scanner] Grading: {zip_path.name}")

    submission = None
    try:
        # Scanner-lab submissions only contain the scanner/ subdir.
        submission = extractor.extract(
            zip_path, required_subdirs=("scanner",), min_subdir_hits=1)
    except Exception as exc:
        tb = traceback.format_exc()
        print(f"[ag-scanner]   EXTRACTION FAILED: {exc}", file=sys.stderr)
        ...
        try:
            ...
            print(f"[ag-scanner]   -> {out_path}  (error stub)")
    ...
    print(f"[ag-scanner]   student: {submission.student_name!r} "
          f"-> {out_path.name}")
    ...
    try:
        ...
        tb = traceback.format_exc()
        print(f"[ag-scanner]   GRADING FAILED (emitting stub): {exc}",
              file=sys.stderr)
        try:
            render_error_stub(zip_path, out_path, tb)
            print(f"[ag-scanner]   -> {out_path}  (error stub)")
    ...
    try:
        ...
        print(f"[ag-scanner]   -> {out_path}  ({graded.percent:.1f}%)")
        if roster is not None:
            roster.append((graded.submission.student_name,
                           graded.percent, out_path.name))
        return out_path
    except Exception as exc:
        print(f"[ag-scanner]   RENDER FAILED: {exc}", file=sys.stderr)
        traceback.print_exc()
        return None
    finally:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(zips) > 1 and roster:
        ...
        try:
            render_overall(roster, overall_path, lab_name="Scanner Lab")
            print(f"[ag-scanner] Batch summary: {overall_path}")
        except Exception as exc:
            print(f"[ag-scanner] Batch summary FAILED: {exc}",
                  file=sys.stderr)

    if failed:
        print(f"[ag-scanner] Done. {failed}/{len(zips)} report(s) failed.")
        return 1
    print(f"[ag-scanner] Done. {len(zips)} report(s) written to {out_dir}.")
    return 0
