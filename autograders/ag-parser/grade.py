#!/usr/bin/env python3
"""
ag-parser: command-line autograder for the ATCS Compilers PARSER lab.

Usage:
    ag-parser STUDENT.zip [-o OUTPUT_DIR]
    ag-parser STUDENTS_DIR/ [-o OUTPUT_DIR]

Reports land in OUTPUT_DIR (default: ./reports/) and are named after
the student's @author tag, falling back to the zip basename. A batch
summary overall.pdf is generated when more than one submission is
processed.
...
def _build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="ag-parser",
        description="ATCS Compilers Parser-lab autograder. Produces a "
...
def _grade_one(zip_path: Path, out_dir: Path, args,
    ...
    print(f"[ag-parser] Grading: {zip_path.name}")

    submission = None
    try:
        # Parser-lab submissions ship scanner/ and parser/ only.
        submission = extractor.extract(
            zip_path,
            required_subdirs=("scanner", "parser"),
            min_subdir_hits=2,
        )
    except Exception as exc:
        tb = traceback.format_exc()
        print(f"[ag-parser]   EXTRACTION FAILED: {exc}", file=sys.stderr)
        ...
        try:
            ...
            print(f"[ag-parser]   -> {out_path}  (error stub)")
    ...
    print(f"[ag-parser]   student: {submission.student_name!r} "
          f"-> {out_path.name}")
    ...
    try:
        ...
        tb = traceback.format_exc()
        print(f"[ag-parser]   GRADING FAILED (emitting stub): {exc}",
              file=sys.stderr)
        try:
            render_error_stub(zip_path, out_path, tb)
            print(f"[ag-parser]   -> {out_path}  (error stub)")
    ...
    try:
        ...
        print(f"[ag-parser]   -> {out_path}  ({graded.percent:.1f}%)")
        if roster is not None:
            roster.append((graded.submission.student_name,
                           graded.percent, out_path.name))
        return out_path
    except Exception as exc:
        print(f"[ag-parser]   RENDER FAILED: {exc}", file=sys.stderr)
        traceback.print_exc()
        return None
    finally:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if in_path.is_file():
        zips = [in_path] if _is_student_zip(in_path) else []
        if not zips:
    ...
    if len(zips) > 1 and roster:
        ...
        try:
            render_overall(roster, overall_path, lab_name="Parser Lab")
            print(f"[ag-parser] Batch summary: {overall_path}")
        except Exception as exc:
            print(f"[ag-parser] Batch summary FAILED: {exc}",
                  file=sys.stderr)

    if failed:
        print(f"[ag-parser] Done. {failed}/{len(zips)} report(s) failed.")
        return 1
    print(f"[ag-parser] Done. {len(zips)} report(s) written to {out_dir}.")
    return 0
