#!/usr/bin/env python3
"""
ag-codegen: command-line autograder for the ATCS Compilers PROCEDURES lab.

Usage:
    ag-codegen STUDENT.zip [-o OUTPUT_DIR]
    ag-codegen STUDENTS_DIR/ [-o OUTPUT_DIR]
...
def _build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="ag-codegen",
        description="ATCS Compilers CodeGen-lab autograder. "
...
def _grade_one(zip_path: Path, out_dir: Path, args,
    ...
    print(f"[ag-codegen] Grading: {zip_path.name}")
    ...
    try:
        ...
        print(f"[ag-codegen]   EXTRACTION FAILED: {exc}", file=sys.stderr)
        ...
        try:
            ...
            print(f"[ag-codegen]   -> {out_path}  (error stub)")
    ...
    print(f"[ag-codegen]   student: {submission.student_name!r} "
          f"-> {out_path.name}")
    ...
    try:
        ...
        # couldn't be graded.
        tb = traceback.format_exc()
        print(f"[ag-codegen]   GRADING FAILED (emitting stub): {exc}",
              file=sys.stderr)
        try:
            render_error_stub(zip_path, out_path, tb)
            print(f"[ag-codegen]   -> {out_path}  (error stub)")
    ...
    try:
        ...
        print(f"[ag-codegen]   RENDER FAILED: {exc}", file=sys.stderr)
        traceback.print_exc()
        return None
    finally:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(zips) > 1 and roster:
        ...
        try:
            ...
            print(f"[ag-codegen] Batch summary: {overall_path}")
        except Exception as exc:
            print(f"[ag-codegen] Batch summary FAILED: {exc}",
                  file=sys.stderr)

    if failed:
        print(f"[ag-codegen] Done. {failed}/{len(zips)} report(s) failed.")
        return 1
    print(f"[ag-codegen] Done. {len(zips)} report(s) written to {out_dir}.")
    return 0
