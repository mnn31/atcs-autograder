#!/usr/bin/env python3
"""
ag-ast: command-line autograder for the ATCS Compilers AST lab.

Usage:
    ag-ast STUDENT.zip [-o OUTPUT_DIR]
    ag-ast STUDENTS_DIR/ [-o OUTPUT_DIR]
...
def _build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="ag-ast",
        description="ATCS Compilers AST-lab autograder. Produces a "
...
def _grade_one(zip_path: Path, out_dir: Path, args,
    ...
    print(f"[ag-ast] Grading: {zip_path.name}")

    submission = None
    try:
        # AST-lab submissions include scanner/, parser/, ast/, environment/.
        # The default extractor threshold of 3 already accepts this.
        submission = extractor.extract(zip_path)
    except Exception as exc:
        tb = traceback.format_exc()
        print(f"[ag-ast]   EXTRACTION FAILED: {exc}", file=sys.stderr)
        ...
        try:
            ...
            print(f"[ag-ast]   -> {out_path}  (error stub)")
    ...
    print(f"[ag-ast]   student: {submission.student_name!r} "
          f"-> {out_path.name}")
    ...
    try:
        ...
        tb = traceback.format_exc()
        print(f"[ag-ast]   GRADING FAILED (emitting stub): {exc}",
              file=sys.stderr)
        try:
            render_error_stub(zip_path, out_path, tb)
            print(f"[ag-ast]   -> {out_path}  (error stub)")
    ...
    try:
        ...
        print(f"[ag-ast]   -> {out_path}  ({graded.percent:.1f}%)")
        if roster is not None:
            roster.append((graded.submission.student_name,
                           graded.percent, out_path.name))
        return out_path
    except Exception as exc:
        print(f"[ag-ast]   RENDER FAILED: {exc}", file=sys.stderr)
        traceback.print_exc()
        return None
    finally:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(zips) > 1 and roster:
        ...
        try:
            render_overall(roster, overall_path, lab_name="AST Lab")
            print(f"[ag-ast] Batch summary: {overall_path}")
        except Exception as exc:
            print(f"[ag-ast] Batch summary FAILED: {exc}", file=sys.stderr)

    if failed:
        print(f"[ag-ast] Done. {failed}/{len(zips)} report(s) failed.")
        return 1
    print(f"[ag-ast] Done. {len(zips)} report(s) written to {out_dir}.")
    return 0
